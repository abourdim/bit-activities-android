#!/usr/bin/env bash
# ============================================================================
#  Bit Activities Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/app-bit-activities}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.bitactivities}"
AVD_NAME="${AVD_NAME:-bit_activities_test}"
...
cmd_test() {
  ...
  local out="/tmp/bit-activities-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Bit Activities Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
